...
def colaboradores(request):
    # Salvar os dados do funcionario no banco de dados
    novo_colaborador = Colaboradores()
    novo_colaborador.nome = request.POST.get('nome')
    novo_colaborador.cpf = request.POST.get('cpf')
    novo_colaborador.email = request.POST.get('email')
    novo_colaborador.endereco = request.POST.get('endereco')
    novo_colaborador.fone = request.POST.get('fone')
    novo_colaborador.save()


def visualizar_colaboradores(request):
    # Exibir os dados dos funcionários cadastrados 
    colaboradores = {
        'colaboradores': Colaboradores.objects.all()
    }
    # Retornar os dados para a página de listagem de dados
    return render(request,'funcionarios/funcionarios.html',colaboradores)
